...
Includes residual analysis, confidence intervals, and model validation
"""
...
class EnhancedTrendSurfaceAlgorithm(QgsProcessingAlgorithm):
    ...
    def processAlgorithm(self, parameters, context, feedback):
        ...
        residual_layer = self.create_residual_points_layer(source, results['residuals'], context)
    ...
    def collect_data(self, source, z_field, weight_field, feedback):
        ...
        return points, z_values, weights if weight_field else None
    ...
    def trend_surface_analysis(self, x, y, z, w, degree, confidence_level, 
        ...
        if robust_regression:
            coefficients, residuals, model_stats = self.robust_regression_fit(A, z, w)
        else:
            coefficients, residuals, model_stats = self.least_squares_fit(A, z, w)
        ...
        confidence_intervals = self.calculate_confidence_intervals(
            A, coefficients, residuals, confidence_level, model_stats
        )
    ...
    def least_squares_fit(self, A, z, w):
    ...
    def robust_regression_fit(self, A, z, w, max_iter=50):
        ...
        for iteration in range(max_iter):
            ...
            robust_weights = np.where(abs_residuals < 1e-10, 1.0, mad / abs_residuals)
            robust_weights = np.minimum(robust_weights, 1.0)  # Huber-like weighting
            
            # Weighted least squares
            W = np.diag(robust_weights)
            A_weighted = W @ A
            z_weighted = W @ z
            
            new_coefficients, new_residuals, rank, s = np.linalg.lstsq(A_weighted, z_weighted, rcond=None)
            
            # Check convergence
            if np.max(np.abs(new_coefficients - coefficients)) < 1e-8:
                break
    ...
    def cross_validation(self, x, y, z, w, degree, feedback, k=5):
        ...
        for fold in range(k):
            ...
            coefficients, _, _ = self.least_squares_fit(A_train, z[train_idx], w_train)
            
            # Test model
            A_test = self.create_design_matrix(x[test_idx], y[test_idx], degree)
            z_pred_test = A_test @ coefficients
    ...
    def calculate_confidence_intervals(self, A, coefficients, residuals, confidence_level, model_stats):
    ...
    def generate_rasters(self, x, y, results, extent, cell_size, crs,
        ...
        for row in range(rows):
            ...
            for col in range(cols):
                ...
                trend_val = self.evaluate_polynomial(x_val, y_val, results['coefficients'], results['degree'])
                trend_data[row, col] = trend_val
                
                # For demonstration, residual is set to 0 (would need observed values everywhere)
                # In practice, you'd interpolate residuals from point locations
                residual_data[row, col] = 0
                confidence_data[row, col] = results['confidence_intervals']['rse']

        # Save rasters
        self.save_raster(output_trend, trend_data, xmin, ymax, cell_size, crs)
        if output_residuals:
            self.save_raster(output_residuals, residual_data, xmin, ymax, cell_size, crs)
        if output_confidence:
            self.save_raster(output_confidence, confidence_data, xmin, ymax, cell_size, crs)
    ...
    def create_residual_points_layer(self, source, residuals, context):
        ...
        features = []
        for i, (feature, residual) in enumerate(zip(source.getFeatures(), residuals)):
            ...
            new_feature.setAttribute(source.fields().count() + 1, float(feature[source.fields().count()] - residual))
    ...
    def generate_statistics(self, output_folder, results, feedback):
        ...
        try:
            ...
            with open(report_path, 'w') as f:
                f.write("POLYNOMIAL TREND SURFACE ANALYSIS REPORT\n")
                f.write("=" * 50 + "\n\n")
                ...
                f.write(f"Residual Mean: {stats['residual_mean']:.4f}\n")
