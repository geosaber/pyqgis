...
class EnhancedTrendSurfacePlugin:
    ...
    def execute_analysis(self):
        ...
        try:
            ...
            weight_field = self.dlg.weightFieldCombo.currentText() or None
            polynomial_degree = self.dlg.polynomialSpin.value()
            cell_size = self.dlg.cellSizeSpin.value()
            confidence_level = self.dlg.confidenceSpin.value()
            cross_validation = self.dlg.crossValidationCheck.isChecked()
            robust_regression = self.dlg.robustRegressionCheck.isChecked()
            ...
            if input_layer.featureCount() < 10:
                QMessageBox.warning(self.dlg, "Input Error", "Insufficient points. Need at least 10 points for analysis.")
                return

            # Get output paths
            output_trend = self.dlg.trendOutputWidget.filePath()
            output_residuals = self.dlg.residualsOutputWidget.filePath() or None
            output_confidence = self.dlg.confidenceOutputWidget.filePath() or None
            output_folder = self.dlg.folderOutputWidget.filePath() or None
            ...
            progress = QProgressDialog("Performing trend surface analysis...", "Cancel", 0, 100, self.dlg)
            progress.setWindowTitle("Enhanced Trend Surface")
            progress.setMinimumDuration(0)
            progress.show()

            def progress_callback(progress_value, progress_text):
                progress.setValue(progress_value)
                progress.setLabelText(progress_text)
                QCoreApplication.processEvents()
                return not progress.wasCanceled()

            # Run the analysis using Processing algorithm
            parameters = {
                'INPUT': input_layer,
                'Z_FIELD': z_field,
            ...
            result = processing.run("enhancedtrendsurface:EnhancedTrendSurfaceAnalysis", 
                                  parameters, feedback=progress)
            
            progress.close()

            if progress.wasCanceled():
                QMessageBox.information(self.dlg, "Analysis Canceled", "Trend surface analysis was canceled.")
                return
            ...
            QMessageBox.information(self.dlg, "Analysis Complete", 
                                  "Trend surface analysis completed successfully!\n\n"
                                  f"Trend surface: {output_trend}\n"
                                  f"Residual points layer added to project.")
